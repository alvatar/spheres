...
(define (parse-arguments args kont options)
  ...
  (let loop ((args args)
    ...
    (define (consume-next-argument!)
      (if (or (null? (cdr args))
              (equal? "--" (cadr args)))
          (arguments/error "Expected an argument to" (car args)))
      (let ((val (cadr args)))
          ...
          (cond
           ...
           (else
            (let* ((opt-chr (string-ref str idx))
              ...
              (if (not opt)
                  (arguments/error "Unrecognized option" (car args)))
              ...
              (let ((val
                     (cond
                      ...
                      ((not (= 2 len))
                       (arguments/error "Option that takes an argument must not be grouped"
                                  (car args)))
   ...
   (lambda (opt)
     (let ((handler (assoc (car opt) handlers)))
       (if handler
           ...
           (arguments/error "Option is not valid in this context:"
                      (cadr opt)))))
   opts))
