...
(define (regexp:string-split str . o)
...
(define (regexp:string-skip str x . o)
  (apply regexp:string-find str (lambda (x) (not (make-char-predicate x))) o))
        ...
        ((#\[)
         (let* ((inv? (eqv? #\^ (string-ref str (+ i 1))))
           ...
           (case (string-ref str i2)
             ((#\:)
              (let ((j (regexp:string-find str #\: (+ i2 1) end)))
                (if (or (>= (+ j 1) end)
           ...
           (cond
            ...
            (else ;; (?...) case
             (case (string-ref str (+ i 2))
               ((#\#)
                (let ((j (regexp:string-find str #\) (+ i 3))))
             ...
             (let ((res (collect/single)))
               (cond
                ...
                (else
                 (let* ((x (car res))
                        (tail (cdr res))
                        (j (regexp:string-find str #\} (+ i 1)))
                        (s2 (regexp:string-split (substring str (+ i 1) j) #\,))
             ...
             (let ((c (string-ref str (+ i 1))))
               (case c
                  ...
                  (let ((c (string-ref str (+ i 2))))
                    (if (not (memv c '(#\< #\{ #\')))
                        ...
                        (let* ((terminal (char-mirror c))
                               (j (regexp:string-find str terminal (+ i 2)))
                 ...
                 (else
                  (cond
                   ((char-numeric? c)
                    (let* ((j (regexp:string-skip str char-numeric? (+ i 2)))
                           (backref
                            (if (flag-set? flags ~case-insensitive?)
                                'backref-ci
                                'backref))
