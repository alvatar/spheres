;; (cond-expand
;;  ((or chicken gambit)
;;   ;; gambit doesn't support define-syntax and chicken doesn't support nested
;;   ;; define-syntax expansions, we have to define our macros using define-macro
;;   ;; and redefine define-syntax and letrec-syntax as doing nothing
;;   (begin
;;     (define-macro (define-syntax name body) #t)
;;     (define-macro (%csv:testeez doc . rest)
;;       `(test* ,@rest))
;;     (define-macro (%csv:error p m o)
;;       `(error (string-append ,p " : " ,m) ,o))
;;     (define-macro (%csv:type-error proc-str expected-str got-value)
;;       `(%csv:error ,proc-str
;;                    (string-append "expected " ,expected-str ", received:")
;;                    ,got-value))
;;     (define-macro (%csv:gosc os-stx)
;;       `(let* ((os ,os-stx)
;;               (str (get-output-string os)))
;;          (close-output-port os)
;;          str))
;;     (define-macro (letrec-syntax head body) body)
;;     (define-macro (newline-check newline-type c port detected-newline-type)
...
;;            (%csv:newline-check-step0 ,newline-type ,c ,port)))
;;     (define-macro (gosc-cons os b)
;;       `(let ((s (%csv:gosc ,os))) (cons s ,b)))
;;     (define-macro (test-define doc arg1 arg2)
;;       `(define ,arg1 ,arg2))
;;     (define-macro (test/equal doc arg1 arg2)
;;       `(expect* (equal? ,arg1 ,arg2)))
;;     #t))
;;  (else #t))

;; macros to help convert testeez tests into snow tests
;; some of the conversion has been done manually
...
;; the original package begins here, snow tests at the bottom
