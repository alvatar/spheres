...
(define (parser-error port . text)
  (error (append-strings text)))
...
(define* (next-token prefix-skipped-chars
                     break-chars
                     (comment "")
...
;;
;; The optional argument PORT defaults to the current input port.
(define *read-line-breaks* (list #\newline #\return #!eof))

(define* (read-text-line (port (current-input-port)))
...
;; The optional argument PORT defaults to the current input port.
(define* (read-string n (port (current-input-port)))
  (if (not (positive? n)) ""
